...
class cswin_Transformer(CSwinAttention_block):
    ...
    def __init__(self, args):
        ...
        # self.CSwinAttention_block = CSwinAttention_block(self.dim, split_size=4, dim_out=None, num_heads=2, attn_drop=0., proj_drop=0., qk_scale=None)
        self.CSwinAttention_block = nn.ModuleList([CSwinAttention_block(args, self.dim, split_size=i, dim_out=None, num_heads=self.num_heads, attn_drop=0., proj_drop=0., qk_scale=None) for i in  args['split_size']])
    ...
    def forward(self, x):
        ...
        for n in range(self.num_layers):
            xo = x
            x_cs_att = self.CSwinAttention_block[n](x)  # 這個 layermorm 在 block 裡面
            x_self = rearrange(x, 'b c h w -> b (h w) c')
            x_self_att, att = self.attn(self.selfln(x_self))    # self layernorm
            x_self_att = x_self_att.permute(0, 2, 1).reshape(b, c, h, w)  # reshape回去
            x = xo + x_cs_att

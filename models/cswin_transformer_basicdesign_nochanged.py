    ...
    def forward(self, x):
        ...
        for n in range(self.num_layers):
            ...
            for _ in range(self.loop_time[n]):
                ...
                x = x_cs_att + x # 這個 layermorm 在 block 裡面
                x1 = rearrange(x, 'b c h w -> b (h w) c')
                x1 = self.mlp1(self.ln_mlp1(x1)).reshape(b, h, w, c).permute(0, 3, 1, 2)
                x = x1 + x
                x_self = rearrange(x, 'b c h w -> b (h w) c')
                x_self_att, att = self.atten(self.selfln(x_self))    # self layernorm
                x_self_att = x_self_att.permute(0, 2, 1).reshape(b, c, h, w)  # reshape回去
                x = x_self_att + x
                x2 = rearrange(x, 'b c h w -> b (h w) c')
                x2 = self.mlp2(self.ln_mlp2(x2)).reshape(b, h, w, c).permute(0, 3, 1, 2)   # mlp2 layernorm
                x = x2 + x

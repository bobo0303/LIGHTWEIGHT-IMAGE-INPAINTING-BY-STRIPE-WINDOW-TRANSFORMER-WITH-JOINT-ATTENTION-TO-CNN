...
'''def BGR2HSV(img, pred_img,n):
    numberofimg_from = n
    numberofimg = n+1
    img = img[numberofimg_from:numberofimg, ...]
    img = img.permute(0, 2, 3, 1) * 255
    img = np.concatenate(img.cpu().detach().numpy().astype(np.uint8), axis=0)  # GT
    hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)

    pred_img = pred_img[numberofimg_from:numberofimg, ...]
    pred_img = pred_img.permute(0, 2, 3, 1) * 255
    pred_img = np.concatenate(pred_img.cpu().detach().numpy().astype(np.uint8), axis=0)  # pre
    pre_hsv = cv2.cvtColor(pred_img, cv2.COLOR_BGR2HSV)
    hsv = FF.to_tensor(hsv).float()
    pre_hsv = FF.to_tensor(pre_hsv).float()

    return hsv, pre_hsv

def BGR2HSV_withedge(img, pred_img, edge, n):
    numberofimg_from = n
    numberofimg = n+1
    edge = edge[numberofimg_from:numberofimg, ...]
    edge = edge.permute(0, 2, 3, 1) * 255
    edge = np.concatenate(edge.cpu().detach().numpy().astype(np.uint8), axis=0)  # edge
    edge_mask = FF.to_tensor(edge).float()
    ...
    return hsv, pre_hsv, edge_mask

def HSV(img, pred_img, batch_size):
    losses = []
    for n in range(batch_size):
        hsv, pre_hsv = BGR2HSV(img, pred_img, n)
        loss = F.mse_loss(hsv, pre_hsv, reduction='none')
        losses.append(loss)
    losses = (torch.stack(losses).sum(dim=0))/8
    losses_H = losses[0].mean()
    losses_S = losses[1].mean()
    losses_V = losses[2].mean()
    losses = (losses_H + losses_S + losses_V)/3

    return losses

def HSV_edge(img, pred_img, edge, batch_size):
    losses = []
    for n in range(batch_size):
        hsv, pre_hsv, edge_mask = BGR2HSV_withedge(img, pred_img, edge, n)
        hsv = hsv * edge_mask
        pre_hsv = pre_hsv * edge_mask
        loss = F.mse_loss(hsv, pre_hsv, reduction='none')
        losses.append(loss)
    losses = (torch.stack(losses).sum(dim=0))/8
    losses_H = losses[0].mean()
...
'''
def HSV_edge(img, pred_img, edge):
    batch = img.shape[0]
    hsv, pre_hsv, edge_mask = BGR2HSV_withedge(img, pred_img, edge, batch)
    hsv = hsv * edge_mask
    pre_hsv = pre_hsv * edge_mask
    loss = F.mse_loss(hsv, pre_hsv, reduction='none')
    losses_H = loss[0].mean()

...
def make_r1_gp(discr_real_pred, real_batch):
    if torch.is_grad_enabled():
        grad_real = torch.autograd.grad(outputs=discr_real_pred.sum(), inputs=real_batch, create_graph=True)[0]  #計算並返回輸出相對於輸入的梯度總和。
        grad_penalty = (grad_real.view(grad_real.shape[0], -1).norm(2, dim=1) ** 2).mean()  #網格懲罰
    else:
        grad_penalty = 0
...
class NonSaturatingWithR1():
    ...
    def generator_loss(self, discr_fake_pred: torch.Tensor, mask=None) \
        ...
        if (self.mask_as_fake_target and self.extra_mask_weight_for_gen > 0) or \
                not self.use_unmasked_for_gen:  # == if masked region should be treated differently 判斷要不要限制遮罩範圍 論文是不限制
    ...
    def discriminator_fake_loss(self, discr_fake_pred: torch.Tensor, mask=None) -> Tuple[torch.Tensor]: #LD 剩下ㄉ

        fake_loss = F.softplus(discr_fake_pred)

        if not self.use_unmasked_for_discr or self.mask_as_fake_target:
            # == if masked region should be treated differently
            mask = self.interpolate_mask(mask, discr_fake_pred.shape[-2:])  #插值遮罩
            # use_unmasked_for_discr=False only makes sense for fakes;
            # for reals there is no difference beetween two regions
            fake_loss = fake_loss * mask    #LD 第三項
            if self.mask_as_fake_target:
                fake_loss = fake_loss + (1 - mask) * F.softplus(-discr_fake_pred)   #LD 第三項 + 第二項

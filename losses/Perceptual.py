...
class PerceptualLoss(nn.Module):
    ...
    def do_normalize_inputs(self, x):
        return (x - self.mean_.to(x.device)) / self.std_.to(x.device)   #以ImageNet設定的均質標準差 (上面那兩個數值可以自己改成對自己資料優化的質)

    def partial_losses(self, input, target, mask=None):
        # input = F.sigmoid(input)    #因輸入圖片非 0~1區間 故使用激勵函數 sigmoid將圖片轉成 0~1區間 by 貿哥支援
        # target = F.sigmoid(target)
        input = (input - torch.min(input)) / (torch.max(input) - torch.min(input))  #正確歸一化方法

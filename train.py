...
# loaded_ckpt
if os.path.exists(args['resume_ckpt']):  # 判斷是否有接續訓練檔
...
for epoch in range(args['train_epoch']):
    ...
    for it, items in enumerate(tqdm(loader, disable=False)):
        ...
        Perceptual_loss = VGG.forward(img, pred_img, mask=mask)  # 有做修改可能會出錯要看loss是否下降 (可以是mask=None)
        ...
        G_loss.backward()  # 反向傳遞 必加第一行
        optimizer.step()  # 優化器更新 地加第二行
